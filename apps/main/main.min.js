...
function watchBat(){setWatch(function(){if(!wOS.awake)wOS.wake();wOS.emit("charging",wOS.isCharging())},D8,{edge:"both",repeat:true,debounce:500})}wOS.init();eval(STOR.read("lcd.js"));var g=ST7789();g.theme=wOS.settings.theme?wOS.settings.theme:{fg:65535,bg:0,fg2:2047,bg2:0,fgH:65535,bgH:31,dark:true};wOS.brightness(wOS.BRIGHT);eval(STOR.read("touch.js"));TC.start();if(wOS.FACEUP&&STOR.read("accel.js")){eval(STOR.read("accel.js"));ACCEL.init();ACCEL.on("faceup",function(){if(!wOS.awake)wOS.wake()})}
wOS.ticker=setInterval(wOS.tick,1E3);wOS.POWER=wOS.isCharging();watchBat();if(STOR.read("alarm.boot.js"))eval(STOR.read("alarm.boot.js"));wOS.btnWatches=[setWatch(function(){if(wOS.awake)load(wOS.settings.clock)},BTN1,{repeat:1})];setWatch(function(){if(wOS.awake)wOS.time_left=wOS.ON_TIME;else wOS.wake()},BTN1,{repeat:true,edge:"falling"});setWatch(function(){if(wOS.awake){wOS.time_left=wOS.ON_TIME;if(!wOS.btnWatches)setTimeout(reset,500)}else wOS.wake()},BTN2,{repeat:true,edge:"falling"});
global.Bangle=wOS;E.getBattery=function(){var v=wOS.batV();v=v<3.7?3.7:v;return Math.floor((v-3.7)*200)};wOS.showLauncher=function(){load("launch.js")};eval(STOR.read("menu.js"));eval(STOR.read("prompt.js"));eval(STOR.read("widgets.js"));