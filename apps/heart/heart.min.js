...
HRS.writeByte(12,2);HRS.writeByte(12,34);HRS.writeByte(1,240);HRS.writeByte(12,2)},read:function(){var m=HRS.readByte(9);var h=HRS.readByte(10);var l=HRS.readByte(15);return m<<8|(h&15)<<4|l&15}};Bangle.setLCDTimeout(300);var x=20;var lasty=120;var lastPeak=0;var interval;var bpminterval;var f1=hpfFilter.filter;var f2=medianFilter.filter;var f3=agcFilter.filter;var f4=lpfFilter.filter;var bf=avgMedFilter.filter;var det=pulseDetector.isBeat;var put=correlator.put;
function doread(){var v=f4(f3(f2(f1(HRS.read()))));v=139-v;v=v>239?239:v<40?40:v;put(v);if(det(v)){var peakTime=Date.now();var bpm=Math.floor(6E4/(peakTime-lastPeak));if(bpm>0&&bpm<200){bpm=bf(bpm);g.setColor(-1).setFontAlign(-1,-1).drawString(" BPM: "+bpm+" ",120,20,true)}lastPeak=peakTime}g.setColor(31).fillRect(x,70,x+1,169);var dv=v/2+50;g.setColor(2016).fillRect(x,lasty,x+1,dv);lasty=dv;x+=2;if(x>=220)x=20}
function showBPM(){var bpm=correlator.bpm();g.setColor(65504).setFontAlign(1,-1).drawString("BPM: "+bpm+" ",120,20,true)}function startMeasure(){if(interval)return;g.clear(1);g.setFont("Vector",16);x=20;g.setColor(31).fillRect(20,70,219,169);HRS.enable();interval=setInterval(doread,40);bpminterval=setInterval(showBPM,2E3)}
function stopMeasure(){if(interval){interval=clearInterval(interval);if(bpminterval)bpminterval=clearInterval(bpminterval);HRS.disable();g.setColor(64832).drawString("PAUSED",120,20,true)}}TC.on("swipe",function(dir){if(dir==TC.RIGHT)startMeasure();else if(dir==TC.LEFT)stopMeasure()});E.on("kill",function(){stopMeasure()});E.showMessage("Swipe right\n to start.\n Swipe left\n to stop.","Heart Rate");